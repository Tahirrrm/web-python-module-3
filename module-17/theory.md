...
### `SERIAL`
Создает автоматически увеличивающиейся числовой id
```sql
CREATE TABLE departments (
    id SERIAL PRIMARY KEY,
    name TEXT NOT NULL
...
```
### `ALWAYS`
Всегда генерирует занчение автоматически
```sql
CREATE TABLE departments (
    id INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    title TEXT NOT NULL,
...
### `VARCHAR`
Хранит текст с определенной ограниченной длиной
```sql
CREATE TABLE users (
    id INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    email VARCHAR(255),
...
### `ENUM` 
список определенных знчений которые могут быть в нашей колонке ограничивает значение заданным набором
```sql
CREATE TYPE order_status AS ENUM ('new', 'paid','shiped','cancelled')
CREATE TABLE orders (
    id INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
...
```
### `THEN`
Указыввает какое условие нужно вернуть если WHEN будет истинным
```sql
CASE
WHEN salary >= 15000 THEN 'high'
WHEN salary >= 80000 THEN 'medium'
